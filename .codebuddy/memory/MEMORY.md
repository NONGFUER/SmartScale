...
- TTS 语速：sherpa-onnx 中生成时若 `genCfg.speed > 0`，按 `length_scale = 1/speed` **覆盖**模型配置 `config.model.matcha.length_scale`（后者仅当 speed==0 生效）。当前 `synthesize()` 内 `genCfg.speed = 1.0204f`（对应 length_scale≈0.98，标准语速略快约 2%），模型 `length_scale` 配置实际被忽略。语速调参改 `genCfg.speed` 即可，`TtsSynthWorker::synthesize` 经 `requestSynthesize` 信号由 `VoiceSpeaker::speak` 触发，未被废弃。
